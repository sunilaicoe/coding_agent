  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        className="max-w-chat rounded-lg border-l-2 border-l-[#098F5F] border border-genesis-elements-borderColor bg-genesis-elements-background-depth-2"
      >
        {/* Header */}
        ...
        <div className="px-4">
          {!isConnected ? (
            <div className="p-3 rounded-md bg-genesis-elements-background-depth-3">
              <span className="text-sm text-genesis-elements-textPrimary">
            ...
            <>
              <div
                className="flex items-center p-2 rounded-md bg-genesis-elements-background-depth-3 cursor-pointer"
                onClick={() => setIsCollapsed(!isCollapsed)}
              >
                <div className="i-ph:database text-genesis-elements-textPrimary mr-2"></div>
                <span className="text-sm text-genesis-elements-textPrimary flex-grow">
                  {description || 'Create table and setup auth'}
                </span>
                <div
                  className={`i-ph:caret-up text-genesis-elements-textPrimary transition-transform ${isCollapsed ? 'rotate-180' : ''}`}
                ></div>
              </div>

              {!isCollapsed && content && (
                <div className="mt-2 p-3 bg-genesis-elements-background-depth-4 rounded-md overflow-auto max-h-60 font-mono text-xs text-genesis-elements-textSecondary">
        ...
        {/* Message and Actions */}
        <div className="p-4">
          <p className="text-sm text-genesis-elements-textSecondary mb-4">{message}</p>

...
export const ToolInvocations = memo(({ toolInvocations, toolCallAnnotations, addToolResult }: ToolInvocationsProps) => {
  ...
  return (
    <div className="tool-invocation border border-genesis-elements-borderColor flex flex-col overflow-hidden rounded-lg w-full transition-border duration-150">
      <div className="flex">
        <button
          className="flex items-stretch bg-genesis-elements-background-depth-2 hover:bg-genesis-elements-artifacts-backgroundHover w-full overflow-hidden"
          onClick={toggleDetails}
          aria-label={showDetails ? 'Collapse details' : 'Expand details'}
        >
          <div className="p-2.5">
            <div className="i-ph:wrench text-xl text-genesis-elements-textSecondary hover:text-genesis-elements-textPrimary transition-colors"></div>
          </div>
          <div className="p-2.5 w-full text-left">
            <div className="w-full text-genesis-elements-textPrimary font-medium leading-5 text-sm">
              MCP Tool Invocations{' '}
              {hasToolResults && (
                <span className="w-full w-full text-genesis-elements-textSecondary text-xs mt-0.5">
        ...
        <AnimatePresence>
          {hasToolResults && (
            <motion.button
              ...
              className="bg-genesis-elements-artifacts-background hover:bg-genesis-elements-artifacts-backgroundHover"
              onClick={toggleDetails}
            >
              <div className="p-2">
                <div
                  className={`${showDetails ? 'i-ph:caret-up-bold' : 'i-ph:caret-down-bold'} text-xl text-genesis-elements-textSecondary hover:text-genesis-elements-textPrimary transition-colors`}
                ></div>
              </div>
            </motion.button>
          )}
      ...
      <AnimatePresence>
        {hasToolCalls && (
          ...
          >
            <div className="bg-genesis-elements-artifacts-borderColor h-[1px]" />

            <div className="px-3 py-3 text-left bg-genesis-elements-background-depth-2">
              <ToolCallsList
        ...
        {hasToolResults && showDetails && (
          ...
          >
            ...
            <div className="p-5 text-left bg-genesis-elements-actions-background">
          ...
          return (
            ...
            >
              <div className="flex items-center gap-1.5 text-xs mb-1">
                {isErrorResult ? (
                  <div className="text-lg text-genesis-elements-icon-error">
                    <div className="i-ph:x"></div>
                  </div>
                ) : (
                  <div className="text-lg text-genesis-elements-icon-success">
                    <div className="i-ph:check"></div>
                  </div>
                )}
                <div className="text-genesis-elements-textSecondary text-xs">Server:</div>
                <div className="text-genesis-elements-textPrimary font-semibold">{annotation?.serverName}</div>
              </div>

              <div className="ml-6 mb-2">
                <div className="text-genesis-elements-textSecondary text-xs mb-1">
                  Tool: <span className="text-genesis-elements-textPrimary font-semibold">{toolName}</span>
                </div>
                <div className="text-genesis-elements-textSecondary text-xs mb-1">
                  Description:{' '}
                  <span className="text-genesis-elements-textPrimary font-semibold">{annotation?.toolDescription}</span>
                </div>
                <div className="text-genesis-elements-textSecondary text-xs mb-1">Parameters:</div>
                ...
                <div className="text-genesis-elements-textSecondary text-xs mt-3 mb-1">Result:</div>
                <div className="bg-[#FAFAFA] dark:bg-[#0A0A0A] p-3 rounded-md">
      ...
      <ul className="list-none space-y-4">
        {toolInvocations.map((tool, index) => {
          ...
          return (
            ...
            >
              <div className="bg-genesis-elements-background-depth-3 rounded-lg p-2">
                <div key={toolCallId} className="flex gap-1">
                  <div className="flex flex-col items-center ">
                    <span className="mr-auto font-light font-normal text-md text-genesis-elements-textPrimary rounded-md">
                      {toolName}
                    </span>
                    <span className="text-xs text-genesis-elements-textSecondary font-light break-words max-w-64">
                  ...
                  <div className="flex items-center justify-end gap-2 ml-auto">
                    <button
                      className={classNames(
                        ...
                        'bg-transparent',
                        'text-genesis-elements-textTertiary hover:text-genesis-elements-textPrimary',
                    ...
                    <button
                      className={classNames(
                        ...
                        'text-accent-500 hover:text-genesis-elements-textPrimary',

...
function ContextMenuItem({ onSelect, children }: { onSelect?: () => void; children: ReactNode }) {
  return (
    <ContextMenu.Item
      onSelect={onSelect}
      className="flex items-center gap-2 px-2 py-1.5 outline-0 text-sm text-gray-900 dark:text-gray-100 cursor-pointer ws-nowrap text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-200 hover:bg-gray-100 dark:hover:bg-white/5 rounded-md"
    >
...
function InlineInput({ depth, placeholder, initialValue = '', onSubmit, onCancel }: InlineInputProps) {
  ...
  return (
    <div
      className="flex items-center w-full px-2 bg-gray-100 dark:bg-gray-800 border border-teal-500 dark:border-teal-400 py-0.5 text-gray-900 dark:text-gray-100"
      style={{ paddingLeft: `${6 + depth * NODE_PADDING_LEFT}px` }}
    >
      <div className="scale-120 shrink-0 i-ph:file-plus text-gray-400 dark:text-gray-500" />
      <input
        ref={inputRef}
        type="text"
        className="ml-2 flex-1 bg-transparent border-none outline-none py-0.5 text-sm text-gray-900 dark:text-gray-100 placeholder:text-gray-400 dark:placeholder:text-gray-500 min-w-0"
    ...
    <>
      <ContextMenu.Root>
        <ContextMenu.Trigger>
          <div
            ...
            className={classNames('relative', {
              'bg-gray-200 dark:bg-gray-700 border border-dashed border-teal-500 dark:border-teal-400 rounded-md':
                isDragging,
            })}
          >
            {children}
        ...
        <ContextMenu.Portal>
          <ContextMenu.Content
            style={{ zIndex: 998 }}
            className="border border-gray-200 dark:border-gray-600 rounded-md z-context-menu bg-white dark:bg-[#252526] data-[state=open]:animate-in animate-duration-100 data-[state=open]:fade-in-0 data-[state=open]:zoom-in-98 w-56"
          >
            <ContextMenu.Group className="p-1 border-b border-solid border-gray-200 dark:border-gray-700">
            ...
            <ContextMenu.Group className="p-1 border-t border-solid border-gray-200 dark:border-gray-700">
              {!isFolder ? (
                <>
    ...
    <FileContextMenu onCopyPath={onCopyPath} onCopyRelativePath={onCopyRelativePath} fullPath={folder.fullPath}>
      <NodeButton
        className={classNames('group', {
          'bg-transparent text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-200 hover:bg-gray-100 dark:hover:bg-white/5':
            !selected,
          'bg-teal-50 dark:bg-white/10 text-teal-700 dark:text-teal-300': selected,
        })}
        depth={folder.depth}
    ...
    <FileContextMenu onCopyPath={onCopyPath} onCopyRelativePath={onCopyRelativePath} fullPath={fullPath}>
      <NodeButton
        ...
        iconClasses={classNames('i-ph:file-duotone scale-98', {
          'group-hover:text-gray-900 dark:group-hover:text-gray-200': !selected,
        })}
        onClick={onClick}
      >
        <div
  ...
  return (
    <button
      className={classNames(
        'flex items-center gap-1.5 w-full pr-2 border-2 border-transparent text-gray-700 dark:text-gray-300 py-0.5',
        className,
      )}
      style={{ paddingLeft: `${6 + depth * NODE_PADDING_LEFT}px` }}
      onClick={() => onClick?.()}

...
import { sidebarToggleStore } from '~/components/header/Header';
...
export const Menu = () => {
  ...
  // Sync with header toggle button
  const [, setTick] = useState(0);
  useEffect(() => {
    return sidebarToggleStore.subscribe(() => setTick((t) => t + 1));
  }, []);
  const open = sidebarToggleStore.isOpen || localOpen;
  const setOpen = (v: boolean | ((prev: boolean) => boolean)) => {
    const newVal = typeof v === 'function' ? v(localOpen) : v;
    setLocalOpen(newVal);
    if (newVal) {
      sidebarToggleStore.open();
    } else {
      sidebarToggleStore.close();
    }
  };
  ...
  const deleteChat = useCallback(
    async (id: string): Promise<void> => {
      ...
      // Clear the project directory in WebContainer if this is the current chat
      if (chatId.get() === id) {
        try {
          await workbenchStore.clearProject();
          console.log('Project directory cleared for deleted chat:', id);
        } catch (clearError) {
          console.error('Failed to clear project directory:', clearError);
        }
      }
    },
    [db],
    ...
    <>
      <motion.div
        ...
        className={classNames(
          ...
          'bg-white dark:bg-gray-950 border-r border-genesis-elements-borderColor',
          'shadow-sm text-sm',
      ...
      >
        <div className="h-12 flex items-center justify-between px-4 border-b border-gray-100 dark:border-gray-800/50 bg-gray-50/50 dark:bg-gray-900/50 rounded-tr-2xl">
          ...
          <div className="flex items-center gap-3">
            <HelpButton onClick={() => window.open('https://stackblitz-labs.github.io/genesis.diy/', '_blank')} />
        ...
        <div className="flex-1 flex flex-col h-full w-full overflow-hidden">
          <div className="p-4 space-y-3">
            <div className="flex gap-2">
              <a
                href="/"
                className="flex-1 flex gap-2 items-center bg-teal-50 dark:bg-teal-500/10 text-teal-700 dark:text-teal-300 hover:bg-teal-100 dark:hover:bg-teal-500/20 rounded-lg px-4 py-2 transition-colors"
              >
            ...
            <div className="relative w-full">
              ...
              <input
                className="w-full bg-gray-50 dark:bg-gray-900 relative pl-9 pr-3 py-2 rounded-lg focus:outline-none focus:ring-1 focus:ring-teal-500/50 text-sm text-gray-900 dark:text-gray-100 placeholder-gray-500 dark:placeholder-gray-500 border border-gray-200 dark:border-gray-800"
                type="search"
          ...
          <div className="flex-1 overflow-auto px-3 pb-3">
            ...
            <DialogRoot open={dialogContent !== null} onOpenChange={(open) => { if (!open) closeDialog(); }}>

      ...
      >
        <div className="flex items-center justify-between gap-4 mt-8 mb-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h4 className="text-md font-medium text-genesis-elements-textPrimary">Cloud Providers</h4>
              <p className="text-sm text-genesis-elements-textSecondary">Connect to cloud-based AI models and services</p>
            </div>
          </div>

          <div className="flex items-center gap-2">
            <span className="text-sm text-genesis-elements-textSecondary">Enable All Cloud</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filteredProviders.map((provider, index) => (
            <motion.div
              key={provider.name}
              className={classNames(
                'rounded-lg border bg-genesis-elements-background text-genesis-elements-textPrimary shadow-sm',
            ...
            >
              <div className="absolute top-0 right-0 p-2 flex gap-1">
                {URL_CONFIGURABLE_PROVIDERS.includes(provider.name) && (
                  <motion.span
                    className="px-2 py-0.5 text-xs rounded-full bg-teal-500/10 text-teal-500 font-medium"
              ...
              <div className="flex items-start gap-4 p-4">
                <motion.div
                  className={classNames(
                    ...
                    'bg-genesis-elements-background-depth-3 group-hover:bg-genesis-elements-background-depth-4',
                    'transition-all duration-200',
                    provider.settings.enabled ? 'text-teal-500' : 'text-genesis-elements-textSecondary',
                  )}
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-4 mb-2">
                    <div>
                      <h4 className="text-sm font-medium text-genesis-elements-textPrimary group-hover:text-teal-500 transition-colors">
                        {provider.name}
                      </h4>
                      <p className="text-xs text-genesis-elements-textSecondary mt-0.5">
                        ...
                        {editingProvider === provider.name ? (
                          <input
                            ...
                            className={classNames(
                              ...
                              'bg-genesis-elements-background-depth-3 border border-genesis-elements-borderColor',
                          ...
                          >
                            <div className="flex items-center gap-2 text-genesis-elements-textSecondary">
                              <div className="i-ph:link text-sm" />
                              <span className="group-hover/url:text-teal-500 transition-colors">
              ...
              <motion.div
                className="absolute inset-0 border-2 border-teal-500/0 rounded-lg pointer-events-none"
                animate={{

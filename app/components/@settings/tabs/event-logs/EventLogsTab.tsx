...
export function EventLogsTab() {
  ...
  const exportAsJSON = () => {
    try {
      ...
      a.href = url;
      a.download = `genesis-event-logs-${new Date().toISOString()}.json`;
  ...
  const exportAsCSV = () => {
    try {
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `genesis-event-logs-${new Date().toISOString()}.csv`;
      document.body.appendChild(a);
      a.click();
      window.URL.revokeObjectURL(url);
      document.body.removeChild(a);
  ...
  const exportAsPDF = () => {
    try {
      ...
      // Add subtitle with genesis.diy
      doc.setFontSize(12);
      doc.setFont('helvetica', 'normal');
      doc.text('GENESIS - AI Development Platform', margin, 45);
      yPos = 70;
      ...
      for (let i = 1; i <= totalPages; i++) {
        ...
        // Add footer text
        doc.text('Generated by GENESIS', margin, doc.internal.pageSize.getHeight() - 10);
      ...
      // Save the PDF
      doc.save(`genesis-event-logs-${new Date().toISOString()}.pdf`);
  ...
  const exportAsText = () => {
    try {
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `genesis-event-logs-${new Date().toISOString()}.txt`;
      document.body.appendChild(a);
      a.click();
      window.URL.revokeObjectURL(url);
      document.body.removeChild(a);
  ...
  const ExportButton = () => {
    ...
    return (
      <DialogRoot open={isOpen} onOpenChange={handleOpenChange}>
        ...
        >
          <span className="i-ph:download text-lg text-gray-500 dark:text-gray-400 group-hover:text-teal-500 transition-colors" />
          Export
        </button>
              ...
              {exportFormats.map((format) => (
                ...
                >
                  ...
                  <div>
                    ...
                    <div className="text-xs text-genesis-elements-textSecondary mt-0.5">
    ...
    <div className="flex h-full flex-col gap-6">
      <div className="flex items-center justify-between">
          ...
          <DropdownMenu.Portal>
            ...
            >
              {logLevelOptions.map((option) => (
                <DropdownMenu.Item
                  key={option.value}
                  className="group flex items-center px-4 py-2.5 text-sm text-gray-700 dark:text-gray-200 hover:bg-teal-500/10 dark:hover:bg-teal-500/20 cursor-pointer transition-colors"
                  onClick={() => handleLevelFilterChange(option.value)}
                >
                  <div className="mr-3 flex h-5 w-5 items-center justify-center">
                    <div
                      className={classNames(option.icon, 'text-lg group-hover:text-teal-500 transition-colors')}
                      style={{ color: option.color }}
                    />
                  </div>
                  <span className="group-hover:text-teal-500 transition-colors">{option.label}</span>
                </DropdownMenu.Item>
              ))}
            </DropdownMenu.Content>
          </DropdownMenu.Portal>
        ...
        <div className="flex items-center gap-4">
          ...
          >
            <span className="i-ph:arrows-clockwise text-lg text-gray-500 dark:text-gray-400 group-hover:text-teal-500 transition-colors" />
            Refresh
          </button>

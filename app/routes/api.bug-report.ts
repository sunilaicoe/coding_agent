...
function formatIssueBody(data: z.infer<typeof bugReportSchema>): string {
  ...
  if (data.includeEnvironmentInfo && data.environmentInfo) {
    ...
    if (data.environmentInfo.genesisVersion) {
      body += `- GENESIS: ${data.environmentInfo.genesisVersion}\n`;
    }
  ...
  body += '---\n*Submitted via GENESIS bug report feature*';
...
export async function action({ request, context }: ActionFunctionArgs) {
  ...
  try {
    ...
    const targetRepo =
      (context?.cloudflare?.env as any)?.BUG_REPORT_REPO || process.env.BUG_REPORT_REPO || 'stackblitz-labs/GENESIS';
    ...
    const octokit = new Octokit({
      auth: githubToken,
      userAgent: 'GENESIS-bug-reporter',
    });

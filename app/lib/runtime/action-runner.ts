...
export class ActionRunner {
  ...
  async #executeAction(actionId: string, isStreaming: boolean = false) {
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        if (attempt > 0) {
          logger.info(`Retrying action ${actionId} (attempt ${attempt}/${maxRetries})`);
        ...
        if (attempt < maxRetries && action.type === 'shell') {
          logger.warn(`Shell action failed (attempt ${attempt + 1}/${maxRetries + 1}), will retry...`);
          continue; // Retry
        }
        ...
        // Don't re-throw — let the pipeline continue so the AI can see the error
        return;
      }
    }
  }
  ...
  async #runShellAction(action: ActionState) {
    ...
    // Auto-rewrite incompatible commands instead of just blocking
    action.content = this.#autoRewriteCommands(action.content);

    // Block any remaining incompatible commands
    const blockedResult = this.#blockIncompatibleCommands(action.content);

    if (blockedResult.blocked) {
      throw new ActionCommandError(blockedResult.reason, blockedResult.suggestion);
    }
  ...
  #blockIncompatibleCommands(command: string): {
    blocked: boolean;
    reason: string;
    suggestion: string;
  } {
    ...
    // List of commands/patterns that are incompatible with WebContainer
    const incompatiblePatterns = [
      {
        pattern: /npx\s+create-react-app/,
        reason: 'create-react-app (CRA) is deprecated and does not work in WebContainer',
        suggestion: 'Use Vite instead:\n  npm create vite@latest -- --template react\nOr create files manually with a Vite setup.',
      },
      {
      ...
      {
        pattern: /npm\s+install\s+react-scripts/,
        reason: 'react-scripts (CRA) is deprecated and does not work in WebContainer',
        suggestion: 'Use Vite instead of react-scripts.',
      },
      {
        pattern: /npx\s+create-next-app/,
        reason: 'create-next-app scaffolding does not work reliably in WebContainer',
        suggestion: 'Create Next.js files manually instead.',
      },
      {
        pattern: /npx\s+create-expo-app/,
        reason: 'create-expo-app may hang in WebContainer',
        suggestion: 'Create Expo files manually instead.',
      },
    ];

    for (const { pattern, reason, suggestion } of incompatiblePatterns) {
      if (pattern.test(trimmedCommand)) {
        logger.warn(`Blocked incompatible command: ${trimmedCommand}`);
  ...
  #autoRewriteCommands(command: string): string {
    let rewritten = command;

    // Auto-rewrite npx create-react-app
    if (/npx\s+create-react-app/.test(rewritten)) {
      const projectNameMatch = rewritten.match(/npx\s+create-react-app\s+(\S+)/);
      const projectName = projectNameMatch ? projectNameMatch[1] : 'my-app';
      rewritten = `mkdir -p ${projectName} && cd ${projectName}`;
      logger.info(`Auto-rewrote: npx create-react-app -> manual Vite setup for ${projectName}`);
    }

    // Auto-rewrite npx create-next-app
    if (/npx\s+create-next-app/.test(rewritten)) {
      const projectNameMatch = rewritten.match(/npx\s+create-next-app\s+(\S+)/);
      const projectName = projectNameMatch ? projectNameMatch[1] : 'my-app';
      rewritten = `mkdir -p ${projectName} && cd ${projectName}`;
      logger.info(`Auto-rewrote: npx create-next-app -> manual setup for ${projectName}`);
    }

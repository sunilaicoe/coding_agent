...
${fixedContent}
  ...
  private _rewriteIncompatibleCommands(command: string): string {
    // Rewrite create-react-app to Vite
    if (/npx\s+create-react-app/.test(command) || /create-react-app/.test(command)) {
      logger.warn('Auto-rewrote create-react-app to Vite setup');
      return 'echo "Using Vite instead of create-react-app" && npm create vite@latest -- --template react';
    }

    // Rewrite react-scripts usage
    if (/npm\s+install\s+react-scripts/.test(command) || /yarn\s+add\s+react-scripts/.test(command)) {
      logger.warn('Blocked react-scripts install');
      return 'echo "react-scripts is not supported. Using Vite instead." && npm install --save-dev vite @vitejs/plugin-react';
    }

    // Rewrite create-next-app
    if (/npx\s+create-next-app/.test(command)) {
      logger.warn('Auto-rewrote create-next-app to manual setup');
      return 'echo "Setting up Next.js manually..."';
    }

...
export async function openDatabase(): Promise<IDBDatabase | undefined> {
  ...
  return new Promise((resolve) => {
    const request = indexedDB.open('genesisHistory', 2);
...
export async function deleteById(db: IDBDatabase, id: string): Promise<void> {
  return new Promise((resolve, reject) => {
    // Try to include snapshots store if it exists, otherwise just chats
    const storeNames: string[] = db.objectStoreNames.contains('snapshots')
      ? ['chats', 'snapshots']
      : ['chats'];
    ...
    // Only delete snapshot if the store exists
    let snapshotDeleted = !db.objectStoreNames.contains('snapshots');

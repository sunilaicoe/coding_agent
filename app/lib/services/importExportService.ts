...
export class ImportExportService {
    ...
    try {
      ...
      return {
        ...
        core: {
          ...
          genesis_user_profile: this._safeGetItem('genesis_user_profile'),
          genesis_settings: this._safeGetItem('genesis_settings'),
          genesis_profile: this._safeGetItem('genesis_profile'),
        ...
        features: {
          // Feature flags
          viewed_features: this._safeGetItem('genesis_viewed_features'),
          developer_mode: this._safeGetItem('genesis_developer_mode'),
        ...
        ui: {
          // Tab configuration
          genesis_tab_configuration: this._safeGetItem('genesis_tab_configuration'),
        ...
        debug: {
          ...
          acknowledged_debug_issues: this._safeGetItem('genesis_acknowledged_debug_issues'),
          acknowledged_connection_issue: this._safeGetItem('genesis_acknowledged_connection_issue'),

          // Error logs
          error_logs: this._safeGetItem('error_logs'),
          genesis_read_logs: this._safeGetItem('genesis_read_logs'),
        ...
        updates: {
          ...
          last_acknowledged_update: this._safeGetItem('genesis_last_acknowledged_version'),
        },
  ...
  private static async _importComprehensiveFormat(data: any): Promise<void> {
    ...
    if (data.ui) {
      ...
      if (data.ui.genesis_tab_configuration) {
        try {
          this._safeSetItem('genesis_tab_configuration', data.ui.genesis_tab_configuration);
        } catch (err) {
    ...
    if (data.debug) {
      ...
      const debugLocalStorageKeys = [
        'genesis_acknowledged_debug_issues',
        'genesis_acknowledged_connection_issue',
        'error_logs',
        'genesis_read_logs',
      ];
    ...
    if (data.updates) {
      ...
      if (data.updates.last_acknowledged_update) {
        try {
          this._safeSetItem('genesis_last_acknowledged_version', data.updates.last_acknowledged_update);
        } catch (err) {

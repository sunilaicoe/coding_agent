...
export class WorkbenchStore {
  ...
  /**
   * Clear the entire project directory in WebContainer.
   * Called when a chat is deleted to clean up the in-browser filesystem.
   */
  async clearProject() {
    try {
      const wc = await webcontainer;

      // List all files/folders in the workdir
      const entries = await wc.fs.readdir(wc.workdir);

      // Remove each entry
      for (const entry of entries) {
        const fullPath = path.join(wc.workdir, entry);

        try {
          await wc.fs.rm(fullPath, { recursive: true });
          logger.info(`Deleted: ${fullPath}`);
        } catch (err) {
          logger.warn(`Failed to delete ${fullPath}:`, err);
        }
      }
      ...
      logger.info('Project directory cleared successfully');
    } catch (error) {
      logger.error('Failed to clear project directory:', error);
    }
  }
  ...
  /**
   * Auto-start fallback: If the AI forgot to include a "start" action,
   * we automatically run `npm install && npm run dev` in the WebContainer.
   * This ensures the preview always shows up.
   */
  async #autoStartIfNeeded(artifactId: string) {
    try {
      const artifact = this.#getArtifact(artifactId);
      if (!artifact) return;
      ...
      // Check if package.json exists (indicating a Node project)
      const wc = await webcontainer;
      try {
        await wc.fs.readFile('package.json', 'utf-8');
      } catch {
        console.log('[Workbench] No package.json found, skipping auto-start');
        return;
      }
  ...
  async _runAction(data: ActionCallbackData, isStreaming: boolean = false) {
    ...
    try {
      ...
      // Don't stop execution on error — log it and continue
      // The action runner already handled retries and status updates
      console.warn(`Action ${data.actionId} failed but continuing execution:`, error);
    }
  }

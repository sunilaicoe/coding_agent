...
import { createFilesContext, extractCurrentContext, extractPropertiesFromMessage, simplifyGenesisActions } from './utils';
...
export async function selectContext(props: {
  ...
  const processedMessages = messages.map((message) => {
    if (message.role === 'user') {
      ...
      content = simplifyGenesisActions(content);

      content = content.replace(/<div class=\\"__genesisThought__\\">.*?<\/div>/s, '');
